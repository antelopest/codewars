...
const parseInt = num => {
  ...
  let amountOfUnits = 0;
  let amountOfHundreds = 0;
  let amountOfMultitudes = 0;

  for (let i = 0; i <= parts.length; i++) {
    if (units[parts[i]]) {
      amountOfUnits = amountOfUnits + units[parts[i]];
      continue;
    }
    if (hundreds[parts[i]]) {
      amountOfHundreds += hundreds[parts[i]] * (amountOfUnits || 1);
      amountOfUnits = 0;
      continue;
    }
    if (multitudes[parts[i]]) {
      amountOfMultitudes += multitudes[parts[i]] * (((amountOfUnits || 0) + (amountOfHundreds || 0)) || 1);
      amountOfUnits = amountOfHundreds = 0;
    }
  }
